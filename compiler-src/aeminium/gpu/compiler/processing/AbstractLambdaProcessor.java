...
public abstract class AbstractLambdaProcessor<T> extends
		AbstractProcessor<CtClass<T>> {
	...
	protected String getOpId(String prefix, CtElement target) {
		String qName = target.getPosition().getCompilationUnit().getMainType()
				.getQualifiedName();
	...
	protected void checkConsistency(CtElement target) {
		if (canSubstitute) {
			...
			ModelConsistencyChecker checker = new ModelConsistencyChecker(
	...
	protected String checkAndGenerateExpr(CtElement expr, String[] input_vars) {
		...
		OpenCLCodeGeneratorVisitor gen = new OpenCLCodeGeneratorVisitor(
				getEnvironment(), input_vars, expr);
		expr.accept(gen);
