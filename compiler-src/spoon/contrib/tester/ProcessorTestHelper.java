...
public class ProcessorTestHelper {
	...
	public static void assertTransform(Processor<?> p, CtFile src,
			CtFile expected) throws Exception {
		...
		if (!expectedFormatted.equals(result)) {
			throw new AssertionError("expected:<" + expectedFormatted
					+ "> but was:<" + result + ">");
		}
	}

	public static CtSimpleType<?> transform(Processor<?> p, CtFile src)
			throws Exception {
		Environment env = new StandardEnvironment();
		env.setVerbose(true);
		Factory factory = new Factory(new DefaultCoreFactory(), env);
	...
	private static CtSimpleType<?> transform(Processor<?> p, Factory factory,
			CtFile src) throws Exception {
		...
		// CtClass<?> ctclass = (CtClass<?>)
		// factory.Type().get(targetClassname);
		// return ctclass;
	...
	public static void prepareBuilder(Builder pbuilder) throws IOException {
		pbuilder.addTemplateSource(new CtFolderFile(new File(
				"compiler-src/aeminium/gpu/compiler/template")));
	}

	private static String transform(Processor<?> p, Factory factory,
			CtFile src, DefaultJavaPrettyPrinter printer) throws Exception {
